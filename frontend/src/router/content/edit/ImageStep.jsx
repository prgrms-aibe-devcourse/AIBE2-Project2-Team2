...
const ImageStep = ({ form, onThumbnailChange, onImagesChange, existingThumbnail, existingImages }) => {
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          {(form.thumbnail || existingThumbnail) ? <CheckCircle className="w-6 h-6 text-green-500" /> : <Circle className="w-6 h-6 text-gray-300" />}
        ...
        <div className="ml-9">
          <div className="relative w-full h-64 border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-all duration-200 group" onClick={() => document.getElementById("main-image-input").click()}>
            {(form.thumbnail || existingThumbnail) ? (
              <div className="relative w-full h-full">
                <img src={form.thumbnail ? thumbnailUrl : existingThumbnail} alt="thumbnail" className="w-full h-full object-cover rounded-lg" />
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          {(form.images && form.images.length > 0) || (existingImages && existingImages.length > 0) ? <CheckCircle className="w-6 h-6 text-green-500" /> : <Circle className="w-6 h-6 text-gray-300" />}
        ...
        <div className="ml-9">
          <div
            ...
            {(form.images && form.images.length > 0) || (existingImages && existingImages.length > 0) ? (
              <div className="w-full">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                  {(form.images && form.images.length > 0 ? imagesUrls : existingImages).slice(0, 5).map((url, idx) => (
