...
export const useCategories = (onCategorySelect) => {
  ...
  const setInitialCategory = (categoryId) => {
    ...
    if (path) {
      if (path.length >= 1) setSelectedCategory1(String(path[0]));
      if (path.length >= 2) setSelectedCategory2(String(path[1]));
      if (path.length >= 3) setSelectedCategory3(String(path[2]));
    }
  };
