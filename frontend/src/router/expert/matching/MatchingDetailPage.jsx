      ...
      <div className="flex-1 flex items-start justify-center pt-36 pb-16">
        <div className="max-w-3xl w-full bg-white rounded-3xl shadow-2xl p-12 flex flex-col gap-8">
          <div className="flex justify-between items-center border-b pb-5">
            ...
            <span className={`px-4 py-2 rounded-xl text-base font-bold ${statusColor[matching.status]}`}>{statusLabel[matching.status] || matching.status}</span>
          </div>
          ...
          <div className="flex flex-wrap gap-4 justify-end pt-6 border-t">
            {matching.status === "ACCEPTED" && (
              <>
                <button className="px-8 py-3 rounded-2xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold text-lg shadow-lg hover:from-indigo-600 hover:to-blue-700 transition-all duration-200" onClick={handleStartWork}>
                  작업 시작
                </button>
                {!rejectMode ? (
                  <button className="px-8 py-3 rounded-2xl bg-gradient-to-r from-pink-500 to-red-400 text-white font-bold text-lg shadow-lg hover:from-red-500 hover:to-pink-400 transition-all duration-200" onClick={() => setRejectMode(true)}>
                    작업 거절
                  </button>
                ) : (
                  <div className="flex gap-2 items-center">
                    <input type="text" placeholder="거절 사유 입력" className="border p-2 rounded focus:ring focus:ring-red-100 w-60" value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} />
                    <button className="px-6 py-2 rounded-xl bg-gradient-to-r from-pink-500 to-red-400 text-white font-bold shadow-md hover:from-red-500 hover:to-pink-400 transition-all duration-200" onClick={handleReject}>
                      거절 확정
                    </button>
                    <button
            ...
            {matching.status === "IN_PROGRESS" && (
              <button className="px-8 py-3 rounded-2xl bg-gradient-to-r from-green-500 to-emerald-400 text-white font-bold text-lg shadow-lg hover:from-emerald-500 hover:to-green-700 transition-all duration-200" onClick={handleCompleteWork}>
                작업 완료
              </button>
            )}
          </div>
