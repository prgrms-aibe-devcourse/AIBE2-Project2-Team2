...
export default function PortfolioEdit() {
    ...
    useEffect(() => {
        if (metaData && category) {
            const foundSpecialty = metaData.detailFields.find(item =>
                item.detailFields.includes(category)
            );
            if (foundSpecialty) {
    ...
    const handleAddImages = async (e) => {
        ...
        const existingNewFilesSize = images
            .filter(img => !img.isExisting && img.file)
            .reduce((acc, cur) => acc + cur.file.size, 0);

        if (existingNewFilesSize + newFilesTotalSize > MAX_TOTAL_SIZE) {
            alert("새로 추가하는 이미지 총 용량은 20MB 이하만 가능합니다.");
            e.target.value = null;
            return;
        }
                        ...
                        <div className="space-y-6">
                                ...
                                {images.length < 4 && (
                                    <div className="mb-6">
                                        ...
                                        <p className="text-xs text-gray-500 mt-2">
                                            PNG, JPG, JPEG 형식 | 최대 5개 | 각 파일 10MB 이하 | 새 파일 총 용량 20MB 이하
                                        </p>
                                    </div>
                                )}
                            ...
                            <div className="pt-2 flex space-x-4">
                                <button
                                    ...
                                    className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold py-3 px-6 rounded-xl transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-gray-200"
                                >
                                    취소
                                </button>
                                <button
