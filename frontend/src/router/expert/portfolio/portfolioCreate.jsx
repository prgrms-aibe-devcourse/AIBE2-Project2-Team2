...
export default function PortfolioCreate() {
  ...
  const handleAddImages = async (e) => {
    ...
    for (const file of files) {
      if (!ALLOWED_TYPES.includes(file.type)) {
        toast.error("PNG, JPG, JPEG만 가능합니다.");
        e.target.value = null;
        return;
      }
    ...
    if (currentTotalSize + newTotalSize > MAX_TOTAL_SIZE) {
      toast.error("총 용량은 20MB 이하만 가능합니다.");
      e.target.value = null;
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      toast.error("필수 항복을 빠짐없이 입력해주세요.");
  ...
  return (
    <div className="py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg backdrop-blur-sm border border-white/20 overflow-hidden">
          <div className="p-6">
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                {images.length < 4 && (
                  <div className="mb-6">
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-2">PNG, JPG, JPEG 형식 | 최대 5개 | 각 파일 10MB 이하 | 총 용량 20MB 이하</p>
                  </div>
                )}
                ...
                {images.length > 0 && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-gray-700">업로드된 이미지 ({images.length}/4)</span>
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between mb-2">
                                <div className="min-w-0 flex-1">
                                  <p className="text-sm font-medium text-gray-900 truncate">{imageObj.file.name}</p>
                                  <p className="text-xs text-gray-500">{(imageObj.file.size / 1024 / 1024).toFixed(2)} MB</p>
                                </div>
