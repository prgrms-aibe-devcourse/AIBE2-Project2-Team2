...
const SkillToggleSelector = ({ meta, selectedSkills, onSkillToggle, error }) => {
  ...
  const isSkillSelected = (categoryName, skillName) => {
    return selectedSkills.some(skill =>
        skill.category === categoryName && skill.name === skillName
    );
  };
  ...
  return (
      <div className="space-y-4">
        <div className="text-sm text-gray-600 bg-slate-50 p-3 rounded-lg border border-slate-200">
          💡 최소 1개, 최대 20개까지 선택 가능합니다. 현재 {getSelectedSkillsCount()}개 선택됨
        </div>
              ...
              .map((category) => {
                ...
                return (
                  ...
                  <span className="font-medium text-gray-800">
                    ...
                    {searchTerm && filteredSkills.length !== category.skills.length && (
                        <span className="ml-2 text-sm text-blue-600">
                        ({filteredSkills.length}개 검색됨)
                      </span>
                    )}
                  </span>
                        <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-500">
                      {selectedSkills.filter(s => s.category === category.categoryName).length}개 선택됨
                    </span>
        ...
        {/* 검색 결과가 없을 때 */}
        {searchTerm && !meta.skills.some(category => shouldShowCategory(category)) && (
              ...
              <div className="flex flex-wrap gap-2">
                {selectedSkills
                  ...
                  {s.category} • {s.name}
                          <button
                              type="button"
                              onClick={() => onSkillToggle(s.category, s.name)}
...
export default function Register() {
  ...
  const handleSkillToggle = (categoryName, skillName) => {
    setForm((prev) => {
      const existingIndex = prev.skills.findIndex(
          skill => skill.category === categoryName && skill.name === skillName
      );
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-slate-50 to-zinc-100 py-8 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-10">
            <h1 className="text-5xl font-bold bg-gradient-to-r from-slate-800 via-zinc-700 to-gray-800 bg-clip-text text-transparent mb-4">
              전문가 등록
            </h1>
          ...
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl border border-gray-200/50 shadow-2xl p-10 space-y-10">
            ...
            <InputGroup label="전문분야" icon={Briefcase} required error={errors.specialties}>
              <div className="space-y-6">
                {form.specialties.map((s, sIdx) => (
                    <div key={sIdx} className="bg-gradient-to-r from-slate-50/80 to-gray-50/80 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/50">
                      <div className="mb-4">
                        <ModernSelect
                            value={s.specialty}
                            onChange={(e) => handleArrayChange(e, sIdx, "specialty", "specialties")}
                            error={errors[`specialty_${sIdx}`]}
                        >
                      ...
                      <div className="space-y-3">
                        ...
                        {s.detailFields.map((d, dIdx) => (
                            <div key={dIdx} className="flex gap-3 items-center">
                              <ModernSelect
                                  value={d}
                                  onChange={(e) => handleDetailFieldChange(e, sIdx, dIdx)}
                                  className="flex-1"
                              >
                ...
                {form.specialties.length < 3 && (
                    <AddButton onClick={() => addArrayItem("specialties", { specialty: "", detailFields: [""] })}>
                      전문분야 추가
                    </AddButton>
                )}
              </div>
            ...
            <InputGroup label="자기소개" icon={User} required error={errors.introduction}>
              <ModernTextarea
                  ...
                  rows={5}
                  placeholder="전문성과 경험을 간략히 소개해주세요..."
              />
            </InputGroup>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <InputGroup label="총 경력 (년)" icon={Briefcase} required error={errors.totalCareerYears}>
                <ModernInput
                    name="totalCareerYears"
                    value={form.totalCareerYears}
                    onChange={handleChange}
                    type="number"
                    min={0}
                    placeholder="5"
              ...
              <div className="space-y-4">
                {form.careers.map((c, idx) => (
                    <div key={idx} className="flex gap-3 items-center">
                      <ModernInput
                          placeholder="경력 내용 (예: ABC 회사에서 3년 근무)"
                          value={c}
                          onChange={(e) => handleArrayChange(e, idx, null, "careers")}
                          className="flex-1"
                      />
                ...
                <AddButton onClick={() => addArrayItem("careers", "")}>
                  경력 추가
                </AddButton>
              </div>
            </InputGroup>
