...
public class AuthService {
    ...
    public boolean signup(SignupRequestDto signupRequestDto) {
        ...
        log.info("회원 가입 완료: {}", user.getEmail());
    ...
    public void kakaoLogin(String code, HttpServletResponse response) {
        ...
        if (userOpt.isPresent()) {
            ...
            loginAndSetCookie(newUser, response);
        }
    }

    private void loginAndSetCookie(User user, HttpServletResponse response) {
        TokenInfo tokenInfo = jwtUtil.createToken(user.getEmail(), "USER");
        redisService.storeActiveToken(tokenInfo.getJti(), user.getEmail(), tokenInfo.getExpirationMs());
        loginService.updateLastLogin(user.getEmail());
